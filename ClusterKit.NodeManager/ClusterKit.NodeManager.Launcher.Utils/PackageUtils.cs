...
namespace ClusterKit.NodeManager.Launcher.Utils
{
    ...
    public static class PackageUtils
    {
            ...
            string nugetUrl)
        {
            ...
            var requiredPackages = Task.WhenAll(packages.Select(async id =>
                {
            ...
            while (true)
            {
                var requirements = requiredPackages.Values
                    ...
                    .Where(ds => 
                        ds.DependencySet != null 
                        && (ds.DependencySet.TargetFramework == null 
                                || NuGetFrameworkUtility.IsCompatibleWithFallbackCheck(framework, ds.DependencySet.TargetFramework)))
                    .SelectMany(ds => ds.DependencySet.Packages)
                    .GroupBy(ds => ds.Id);

                var additionalPackagesTasks = requirements
                    .Select(
                        async r =>
                            {
                                if (requiredPackages.TryGetValue(r.Key.ToLower(), out var currentPackage)
                                    && r.All(rc => rc.VersionRange.Satisfies(currentPackage.Identity.Version)))
                                {
                                ...
                                while (await enumerator.MoveNextAsync())
                                {
                                    ...
                                    if (r.Key == enumerator.Current.Identity.Id && r.All(rq => rq.VersionRange.Satisfies(p.Identity.Version)))
                                    {
