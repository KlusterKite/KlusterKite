// --------------------------------------------------------------------------------------------------------------------
// <copyright file="PropertyResolverGenerator.cs" company="ClusterKit">
//   All rights reserved
// </copyright>
// <summary>
...
namespace ClusterKit.API.Provider
{
    ...
    /// </summary>
    /// <remarks>
    /// IResolver gets the type field value for the request. So for each published property in each published type we should have personal resolver
    /// </remarks>
    internal class PropertyResolverGenerator
    {
        /// <summary>
        ...
        /// The data.
        /// </param>
        public PropertyResolverGenerator(MemberInfo member, TypeMetadata metadata, Type sourceType, AssembleTempData data)
        {
            this.Member = member;
        ...
        /// </summary>
        public string ClassName => $"ClusterKit.API.Provider.Dynamic.IResolver{this.Uid:N}";
        ...
        public virtual string Generate()
        {
            ...
            var code = $@"
                    ...
                    {{
                        ...
                        public class IResolver{this.Uid:N} : PropertyResolver 
                        {{
                            ...
                            {this.GenerateGetValueRecursive()}
        ...
        /// <summary>
        /// Generates code for <see cref="PropertyResolver.GetValueRecursive"/>
        /// </summary>
        /// <returns>The generated code</returns>
        protected virtual string GenerateGetValueRecursive()
        {
            string recursivePart;
            if (this.Metadata.IsForwarding 
                || this.Metadata.MetaType != TypeMetadata.EnMetaType.Object
                || this.Metadata.Type.IsSubclassOf(typeof(Enum)))
            {
                recursivePart = "throw new NotImplementedException();";
            }
            else
            {
                recursivePart = $@"
                    var nextElement = path.Peek();
                    var nextResolver = FieldResolvers[nextElement.FieldName];
                    return await nextResolver.GetValueRecursive(resultSource, path, context, argumentsSerializer);
                ";
            }

            return $@"
                public override async Task<object> GetValueRecursive(object source, Queue<ApiRequest> path, RequestContext context, JsonSerializer argumentsSerializer)
                {{
                    var pathElement = path.Dequeue();
                    var resultSource = await this.GetValue(source, pathElement, context, argumentsSerializer);
                    if (path.Count == 0) 
                    {{
                        return resultSource;
                    }}
        ...
        protected virtual string GenerateObjectHelpers(ApiObjectType apiObjectType)
        {
            ...
            {{
                ""{f.Name}"", new {this.Data.ResolverNames[apiObjectType.TypeName][f.Name]}()
                    
            }}
            ");

            return $@"
            private static Dictionary<string, PropertyResolver> FieldResolvers 
                = new Dictionary<string, PropertyResolver>
            {{
        ...
        protected virtual string GenerateObjectResolve(string itemName, Type type, string queryVariableName = "query", bool generateRequestData = true)
        {
            ...
            return $@"
                ...
                {{
                    result.Add(field.Alias ?? field.FieldName, await FieldResolvers[field.FieldName].Resolve({itemName}, field, context, argumentsSerializer, onErrorCallback));
                }}
                {requestData}
            ";
        }
